  ...
  const pick = useCallback(
    (node: number) => {
      setStart((s) => {
        ...
        // si ya hay inicio y no meta -> set meta; si hay ambos -> reiniciar
        if (goal === null) {
...
function Body({
  ...
  if (loading) {
    return (
      <div className="flex h-full items-center justify-center p-8">
        <p className="text-sm text-zinc-400">Descargando el mapa de Salto…</p>
      </div>
    );
  }
  if (error || !graph) {
  ...
  // Falta elegir puntos: mapa interactivo + banner.
